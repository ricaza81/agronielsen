...
//login
        ...
        protected function landing()
    {
        ...
        ->with("departamentos",$departamentos)
         ->with("cultivos",$cultivos);
          ...
          protected function precipitacion()
    {
        //$productorecomendado = ProductoRecomendado::where('id','=',2)->latest()->first();
        $paises=Pais::all();
        $cultivos=Cultivos::all();
         ...
         protected function consultaclima()
    {
        ...
        ->with("date",$date)
        ->with("canales",$canales)
        ->with("consultasclima",$consultasclima)
        ->with("departamentos",$departamentos)
        ->with("cultivos",$cultivos);

    }
    
     protected function comprarhistorial()
    {
        //$productorecomendado = ProductoRecomendado::where('id','=',2)->latest()->first();
        $params1="";
        $paises=Pais::all();
     ...
     protected function comprarhistorialglobo()
    {
        //$productorecomendado = ProductoRecomendado::where('id','=',2)->latest()->first();
        $params1="";
        $paises=Pais::all();
        ...
        return view('comprarhistorialglobo')
        ->with("params1",$params1)
        ->with("paises",$paises)
        //->with("productorecomendado",$productorecomendado)
        ->with("zonas",$zonas)
  ...
  protected function calculo_req_cultivo()
    {

        $paises=Pais::all();
        $cultivos=Cultivos::all();
        //$cultivos=Cultivos::where(count(cultivo_nutrientes)!=0);
        $zonas=Zonas::all();
        $canales=Canales::all();
        $departamentos=Departamentos::all();

        return view('calculo_req_cultivo')
        ->with("paises",$paises)
        ->with("zonas",$zonas)
        ->with("canales",$canales)
        ->with("departamentos",$departamentos)
        ...
        public function requerimiento_front($id)
        {
       
       $plan_requerimiento=CultivosNutrientesRequerimiento::where("idCultivo","=",$id)->get();
      //$prospecto=User::all();
        return response ()->json($plan_requerimiento);
      
          }

              public function requerimiento_img_cultivo($id)
        {
       
       //$thumb_cultivo=Cultivos::where("id","=",$id)->select('thumb')->get()->first();
       $thumb_cultivo=Cultivos::where("id","=",$id)->get();
      //$prospecto=User::all();
        return response ()->json($thumb_cultivo);
 ...
 public function redirect()
   ...
   $reglas = array('nombres' => 'required',
    ...
    protected function newclima(Request $request)
    {
        $data=$request->all();

       // $reglas=array ('url' => 'required|url');
        //$asunto="Informe variables meteorologicas";
        $consulta= new ConsultaClima;
        $consulta->latitud  =  $data["latitud"];
        $consulta->longitud  =  $data["longitud"];
        $consulta->link  =  $data["url"];
        $consulta->email  =  $data["email_report"];
        $resul= $consulta->save();

        $consultaclima = DB::table('consultaclima')->latest()->first();

        $asunto="Pronostico variables meteorologicas";
        $destinatario=$request->input('email_report');
        $consulta= DB::table('consultaclima')->latest()->first();
        ...
        Mail::send('correo.plantilla_informe_clima', $data, function ($message) use ($asunto,$destinatario,$latitud,$longitud,$url) {
        ...
        $longitud  =  $data["longitud"];
        $msj=NULL;
                 return view('resultclima')
                 //return view('notificationclima')
                 //return redirect('notificationclima/'.$url)
                 ->with('latitud',$latitud)
                 ->with('longitud',$longitud)
        ...
        public function notificationclima(Request $request)
            {
              
              $data=$request->all();
              $id_consulta=$request->input('id_consulta');
              $consulta = ConsultaClima::find($id_consulta);
              $consulta->email=$data["email_report"];
              $resul=$consulta->save();
              ...
              $asunto="Informe variables meteorologicas";
              $destinatario=$request->input('email_report');
              $latitud=$consultaclima->latitud;
              $longitud=$consultaclima->longitud;
              $url  =  $consultaclima->link;
...
public function postLogin(Request $request)
   {
    ...
    if ($this->auth->attempt($credentials, $remember))
        ...
        /*Redirecciones por condiciones*/
                  ...
                  $fincas=AgriFincas::where('idUsuario','=', \Auth::user()->id)->get();
                  $cantfincas=count($fincas);
                  $fincascompartidas=AgriFincasCompartidas::where('id_usuario_compartido','=', \Auth::user()->id)->get();
                  $cantfincascompartidas=count($fincascompartidas);
                  ...
                  $cantpdtossinprecio=count($prdtossinprecio);
                    ...
                    //if ($cantpdtos>0) {return redirect('listado_graficas');}
                    if ($cantpdtos>0) {return redirect('listado_fincas_agricultores_compartidas');}
                    if(\Auth::user()->id==1110528441){return redirect('listado_fincas_agricultores');}
                    //if ($cantpdtos==0) {return redirect('formnuevoproductoempresa');}
                    if ($cantfincascompartidas>0) {return redirect('listado_fincas_agricultores_compartidas');}
